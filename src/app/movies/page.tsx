          ...
          {movies?.map(movie => (
            <li key={movie._id.toString()} className='relative'>
              <div className='group block aspect-square w-full overflow-hidden rounded-lg bg-gray-100'>
                <Image
                  src={imagePath + movie.posterPath}
                  alt=''
              ...
              <p className='block text-sm font-medium text-gray-500'>{movie.overview}</p>
              <p className='block text-sm font-medium text-gray-500'>{movie.releaseDate}</p>
            </li>
          ))}
        </ul>
      </div>
